...
FlickrRequest.prototype.send = function() {
  if (this.method === 'upload' || this.method === 'flickr.photos.setTags')
    this.sendPOST();
  else
    this.sendGET();
};
...
FlickrRequest.prototype.sendPOST = function() {
  ...
  var api_url = this.client.oauth_client.signUrl('http://api.flickr.com/services/upload/' + this.queryString(),
  ...
  form.append('photo', this.params.photo, {
    filename: this.params.title + '.png',
    contentType: 'image/png',
    knownLength: this.params.photo.length
  });
  ...
  console.log('sending')
  var req = http.request(payload, function(res) { console.log('d'); self.handleResponseStream(res); });
  console.log('next')
  form.pipe(req);
};
